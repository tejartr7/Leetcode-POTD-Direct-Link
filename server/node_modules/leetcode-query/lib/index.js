...
var LeetCode = class extends EventEmitter2 {
  ...
  async submission(id) {
    ...
    try {
      ...
      const res = await fetch2(`${BASE_URL}/submissions/detail/${id}/`, {
        headers: {
          origin: BASE_URL,
          referer: BASE_URL,
  ...
  async graphql(query) {
    ...
    try {
      await this.limiter.lock();
      const BASE = BASE_URL;
      const res = await fetch2(`${BASE}/graphql`, {
        method: "POST",
        headers: {
...
async function get_csrf2() {
  const res = await fetch3(`${BASE_URL_CN}/graphql/`, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      "user-agent": USER_AGENT
...
var LeetCodeCN = class extends EventEmitter3 {
  ...
  async graphql(query, endpoint = "/graphql/") {
    ...
    try {
      ...
      const res = await fetch4(`${BASE}${endpoint}`, {
        method: "POST",
        headers: {
          "content-type": "application/json",
          origin: BASE,
